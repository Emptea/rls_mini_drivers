...
#define XILINX_MCDMA_CHAN_PKTDROP_OFFSET(x)	(0x58 + (x) * 0x40)
...
#define XILINX_MCDMA_PKTDROP_MASK       GENMASK(15, 0)
				  ...
				  struct xilinx_dma_tx_descriptor *desc)
{
	...
	list_for_each(entry, &desc->segments) {
		if (chan->xdev->dma_config->dmatype == XDMA_TYPE_CDMA) {
			...
			u32 status;
			u32 mask = chan->xdev->max_buffer_len;
			...
			status = (chan->direction == DMA_DEV_TO_MEM) ?
				 READ_ONCE(aximcdma_hw->status) :
				 READ_ONCE(aximcdma_hw->sideband_status);

			residue += (aximcdma_hw->control & mask) -
				   (status & mask);
		}
	}
...
static u32 xilinx_mcdma_collect_drops_locked(struct xilinx_dma_chan *chan)
{
    ...
    /* Read-to-clear: reading consumes the hardware count. */
    dropped = dma_ctrl_read(
        chan, XILINX_MCDMA_CHAN_PKTDROP_OFFSET(chan->tdest));
    dropped &= XILINX_MCDMA_PKTDROP_MASK;

    chan->rx_dropped_total += dropped;
    return dropped;
}
...
static void xilinx_dma_complete_descriptor(struct xilinx_dma_chan *chan)
{
	...
	list_for_each_entry_safe(desc, next, &chan->active_list, node) {
		if (chan->xdev->dma_config->dmatype == XDMA_TYPE_AXIDMA) {
			...
			status = (chan->direction == DMA_DEV_TO_MEM) ?
				 READ_ONCE(seg->hw.status) :
				 READ_ONCE(seg->hw.sideband_status);
...
static irqreturn_t xilinx_mcdma_irq_handler(int irq, void *data)
{
	...
	if (status & XILINX_MCDMA_IRQ_IOC_MASK) {
		...
		dropped = xilinx_mcdma_collect_drops_locked(chan);
    	total = chan->rx_dropped_total;
		spin_unlock(&chan->lock);
		if (dropped){
        dev_warn_ratelimited(chan->dev,
                            "S2MM TDEST=%u: dropped +%u, total=%llu\n",
                            (unsigned int)chan->tdest,
                            dropped,
